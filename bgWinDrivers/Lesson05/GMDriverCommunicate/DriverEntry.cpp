...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT driver_object, IN PUNICODE_STRING  registry_path)
{
	...
	KdPrint(("[GMDriverCommunicate] Hello World !"));
	...
	// 这里我们只设置1个派发函数
	driver_object->MajorFunction[IRP_MJ_DEVICE_CONTROL] = NtDriverDeviceIoControlFunction;

	// 接下来创建驱动的设备对象
	UNICODE_STRING device_name;
	RtlInitUnicodeString(&device_name, L"\\Device\\GMDriverCommunicate");
	...
	{
		KdPrint(("[GMDriverCommunicate] Create device object failed, status : 0x%X", statue));
		return statue;
	}

	UNICODE_STRING symbolink_name;
	RtlInitUnicodeString(&symbolink_name, L"\\??\\GMDriverCommunicate");
	...
	if (!NT_SUCCESS(statue))
	{
		KdPrint(("[GMDriverCommunicate] Create symbolink name failed, status : 0x%X", statue));
		return statue;
	}
...
VOID DriverUnload(IN PDRIVER_OBJECT driver_object)
{
	...
	do {
		...
		{
			KdPrint(("[GMDriverCommunicate] DriverUnload device object pointer is NULL ..."));
			break;
		}
		...
		{
			KdPrint(("[GMDriverCommunicate] DriverUnload delete symbolinke name %wZ", &device_extension->device_symbolink_name));
		}
...
{
	KdPrint(("[GMDriverCommunicate] NtDriverDispatchFunction"));
